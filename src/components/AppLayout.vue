...
import { backendEnabled, apiHealth, apiGetSnapshots, apiGetSnapshot } from '@/utils/api';
...
// ---- History: what was done (activity) + restore points (saved versions) ----
const showHistory = ref(false); const snapshots = ref([]); const loadingSnaps = ref(false); const confirmVersion = ref(null);
const backendOn = backendEnabled();
function actCat(text) {
...
function openHistory() { confirmVersion.value = null; showHistory.value = true; if (backendOn) { loadingSnaps.value = true; snapshots.value = []; apiGetSnapshots(30).then((r) => { snapshots.value = Array.isArray(r) ? r : []; }).catch(() => {}).finally(() => { loadingSnaps.value = false; }); } }
async function restoreSnap(v) { try { const snap = await apiGetSnapshot(v); if (snap && snap.state) { store.restoreState(snap.state, v); toast.success('Restored the workspace to version ' + v + '.'); showHistory.value = false; } else { toast.error('That version has no saved data.'); } } catch (e) { toast.error('Restore failed: ' + (e.message || e)); } confirmVersion.value = null; }
...
<template>
  <div class="min-h-screen bg-slate-50">
    ...
    <div class="lg:pl-64">
      <header class="sticky top-0 z-30 flex items-center h-16 bg-white/90 backdrop-blur border-b border-slate-200 px-4 sm:px-6 lg:px-8">
        ...
        <div class="ml-auto flex items-center gap-3">
          ...
          <button class="inline-flex h-9 w-9 items-center justify-center rounded-full hover:bg-slate-100 text-slate-600 transition-colors" title="History — what was done + restore points" @click="openHistory">
    ...
    <!-- History: what was done + restore points (step back to an earlier saved version) -->
    <Modal v-if="showHistory" title="History" sub="Everything that's been done — and restore points to step back if you need to adjust." wide @close="showHistory=false">
      <div class="grid gap-5 md:grid-cols-2">
        <div>
          <div class="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">Recent actions <span class="text-slate-400 font-normal normal-case">— what was done</span></div>
          <div class="space-y-1.5 max-h-[55vh] overflow-y-auto pr-1">
            <div v-for="a in store.activity" :key="a.id" class="flex items-start gap-2 rounded-lg border border-slate-100 px-3 py-2">
              <Badge :tone="actCat(a.text).tone">{{ actCat(a.text).label }}</Badge>
              <div class="flex-1 min-w-0"><div class="text-sm text-slate-700">{{ a.text }}</div><div class="text-[10px] text-slate-400">{{ fmtDateTime(a.at) }}</div></div>
            </div>
            <p v-if="!store.activity.length" class="text-center text-slate-400 text-sm py-6">No actions recorded yet.</p>
          </div>
        </div>
        <div>
          <div class="text-xs font-semibold uppercase tracking-wide text-slate-500 mb-2">Restore points <span class="text-slate-400 font-normal normal-case">— saved versions</span></div>
          <div v-if="!backendOn" class="text-sm text-slate-400 rounded-lg border border-dashed border-slate-200 p-4 text-center">Version restore is available when connected to the shared database.</div>
          <div v-else class="space-y-1.5 max-h-[55vh] overflow-y-auto pr-1">
            <p v-if="loadingSnaps" class="text-sm text-slate-400 py-4 text-center">Loading versions…</p>
            <div v-for="s in snapshots" :key="s.version" class="flex items-center gap-2 rounded-lg border border-slate-100 px-3 py-2">
              <div class="flex-1 min-w-0"><div class="text-sm font-medium text-slate-700">Version {{ s.version }}</div><div class="text-[10px] text-slate-400">{{ fmtDateTime(s.createdAt) }}</div></div>
              <button v-if="confirmVersion===s.version" class="text-[11px] font-bold text-white bg-rose-600 px-2 py-1 rounded hover:bg-rose-700" @click="restoreSnap(s.version)">Confirm restore</button>
              <button v-else class="text-xs font-semibold text-indigo-600 hover:underline" @click="confirmVersion=s.version">Restore</button>
            </div>
            <p v-if="!loadingSnaps && !snapshots.length" class="text-center text-slate-400 text-sm py-6">No saved versions yet.</p>
          </div>
          <p class="text-[10px] text-slate-400 mt-2">Restoring reverts the whole workspace to that point, then saves as a new version — so you can always step forward again.</p>
        </div>
      </div>
    </Modal>
  </div>
