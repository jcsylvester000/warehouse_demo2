    ...
    <Modal v-if="showStart" title="Start a return" sub="Where is it coming from? Search and pick the assets." wide @close="showStart=false">
      <div class="space-y-3">
        <div class="grid grid-cols-3 gap-3">
          ...
          <label class="text-sm"><span class="block text-slate-600 mb-1">From SO (optional)</span><select v-model="start.so_ref" class="w-full h-9 px-3 rounded-lg border border-slate-300 text-sm bg-white"><option value="">— none —</option><option v-for="so in shippedSOs" :key="so.id" :value="so.so_number">{{ so.so_number }}<template v-if="so.shipping_address"> · {{ String(so.shipping_address).split(' · ')[0] }}</template></option></select></label>
        </div>
