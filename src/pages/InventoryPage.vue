  ...
  <div>
    ...
    <Modal v-if="showDetail && detail" :title="detail.name" :sub="'Item # ' + detail.sku" wide @close="showDetail=false">
      <div v-if="detailItem" class="space-y-4">
        <img v-if="detailItem.image" :src="detailItem.image" alt="item photo" class="w-full max-h-56 object-contain rounded-lg ring-1 ring-slate-200 bg-slate-50 cursor-zoom-in" title="Click to enlarge" @click="lb.open(detailItem.image, detailItem.name)" />
      ...
      <div v-else-if="detailGroup" class="space-y-4">
        <img v-if="detailGroup.image" :src="detailGroup.image" alt="group photo" class="w-full max-h-56 object-contain rounded-lg ring-1 ring-slate-200 bg-slate-50 cursor-zoom-in" title="Click to enlarge" @click="lb.open(detailGroup.image, detailGroup.name)" />
    ...
    <Modal v-if="showAdd" :title="(itemForm.id||groupForm.id||asmForm.id)?'Edit':'Add to inventory'" :sub="(itemForm.id||groupForm.id||asmForm.id)?'':'Choose a single item, a group, or an assembly.'" wide @close="showAdd=false">
      <div class="space-y-4">
        ...
        <div v-if="addKind==='item'" class="grid grid-cols-2 gap-4">
          ...
          <label class="text-sm flex items-center gap-2 col-span-2"><input v-model="itemForm.assembly_only" type="checkbox" /> This item can only be shipped as an asset <ReqTag ver="V4" code="IT-2" text="Amendment — an assembly-only item (laptop, gameshow) can't be added to a Sales Order as a loose item; it must be assembled first. A Single is never an asset." /></label>
