    ...
    <Modal v-if="showForm" :title="editing ? ('Edit ' + (store.purchaseOrders.find(p=>p.id===form.id)||{}).po_number) : 'New Purchase Order'" wide @close="afterFormClose()">
      <div class="space-y-4">
        <div class="grid grid-cols-3 gap-4">
          ...
          <label class="text-sm"><span class="block text-slate-600 mb-1">Deposit <ReqTag code="PO-1" text="V3 PO #1 — Deposit auto-fills from the vendor's deposit rule and stays editable." /> <ReqTag ver="V4" code="PO-3" text="V4 PO #3 — Deposit now calculates correctly off the bill total via vendor rules (set the % in Manage Vendors)." /> <span class="text-[10px] text-slate-400">auto {{ depositSuggestPct }}% of total</span></span><div class="relative"><span class="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-slate-500 text-sm">$</span><input v-model.number="form.deposit" type="number" step="0.01" class="w-full h-9 pl-6 pr-3 rounded-lg border border-slate-300 text-sm" @input="depositTouched=true" /></div></label>
        </div>
